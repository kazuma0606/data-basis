...
### ✅ フェーズ-1 完了基準
- [x] `pre-v1.3` がスナップショット一覧に表示されること
...
- [x] **1-1. monitoring namespace 作成**
  ```bash
  kubectl create namespace monitoring
  ```

- [x] **1-2. Prometheus マニフェスト作成・適用**
...
### ✅ フェーズ1 完了基準
- [x] `kubectl get pods -n monitoring` で prometheus / node-exporter が Running
- [x] Prometheus UI の Targets 画面で node-exporter が `State: UP`
- [x] `node_cpu_seconds_total` クエリが結果を返すこと
...
### ✅ フェーズ2 完了基準
- [x] Prometheus UI で `kube_pod_status_phase{namespace="default"}` が全 Pod 分返ること
- [x] `kube_deployment_status_replicas_available` で全 Deployment の状態が確認できること
...
- [x] **3-1-1. kafka-exporter マニフェスト作成・適用**
...
- [x] **3-2-1. postgres_exporter マニフェスト作成・適用**
...
- [x] **3-3-1. clickhouse_exporter マニフェスト作成・適用**
...
- [x] **3-4-1. redis_exporter マニフェスト作成・適用**
...
### ✅ フェーズ3 完了基準
- [x] Prometheus Targets 画面で kafka / postgres / clickhouse / redis exporter が全て `UP`
- [x] 各サービスの `*_up` メトリクスが 1 を返すこと
...
### ✅ フェーズ4 完了基準
- [x] Grafana に 6 枚のダッシュボードが表示されること
- [x] 全パネルにデータが入っていること（`No data` がないこと）
...
### ✅ フェーズ5 完了基準
- [x] Alertmanager UI で routing ツリーが正しく表示されること
- [x] テストアラートが Slack に届くこと
...
- [x] **6-3. Grafana バッチジョブダッシュボードにデータが表示されることを確認**

### ✅ フェーズ6 完了基準
- [x] バッチ実行後に Pushgateway UI（`:9091`）にメトリクスが表示されること
- [x] Grafana のバッチジョブダッシュボードで最終成功時刻・実行時間が確認できること
- [x] `BatchJobStalenessDaily` アラートが 25h 後に発火すること（手動テスト）
...
- [x] **7-1. FastAPI に Prometheus メトリクスミドルウェアを追加**
...
- [x] **7-3. SLO / エラーバジェットダッシュボード作成**
  - 直近 30 日の SLO 充足率
  - エラーバジェット残量（%）
  - バジェット消化速度（burn rate）

- [x] **7-4. pyproject.toml に prometheus-fastapi-instrumentator を追加**
  ```bash
...
### ✅ フェーズ7 完了基準
- [x] Grafana SLO ダッシュボードで 4 つの SLO 全てにデータが表示されること
- [x] エラーバジェット残量が計算・表示されること
...
- [x] **8-4. `vagrant snapshot save "v1.3-stable"`**
  ```bash
  cd infrastructure/vagrant/production
...
- 完了日: 2026-03-21
- 注記:
  - ClickHouseエクスポーター: 設定ファイル競合により scrape 無効化（clickhouse-exporter/manifest.yaml は作成済み）
  - Alertmanager Slack Webhook URL は PLACEHOLDER のまま（本番運用時に設定が必要）
